...
def get_tenant_model(apps: Apps = apps) -> TenantType:
    try:
        ...
        raise ImproperlyConfigured("OCCUPATION_TENANT_MODEL is not set: is 'occupation' in your INSTALLED_APPS?")
    except ValueError:
        raise ImproperlyConfigured("OCCUPATION_TENANT_MODEL must be of the form 'app_label.model_name'.")
    except LookupError:
...
def enable_row_level_security(app_label: str, model_name: str, apps: Apps = apps, superuser: bool = False) -> None:
    ...
    data = {"table_name": model._meta.db_table, "policy": " AND ".join(policy_clauses)}
